...
class SearchResult extends Component {
  ...
  _onRenderItem(item) {
    ...
    MapDetailPage_onRenderItem(item) {
    // switch(item.type) {
    //   case 'user':
    // return this._onUserItem(item)
    //   case 'place':
    return this.MapDetailPage_onPlaceItem(item)
    //   case 'campaign':
    //     return this._onCampaignItem(item)
    // }
    }
    MapDetailPage_onPlaceItem(item) {
      console.log(this.props);
      return (
        <View style={styles.item}>
          <Image source={require('@assets/images/greenPin.png')} style={styles.placeImage} />
          <View style={styles.infomation}>
            <TouchableOpacity onPress={() => this.props._locationSelected(item)}>
              <Text style={styles.name}>{item.primaryText}</Text>
              <Text style={styles.following}>{item.secondaryText}</Text>
            </TouchableOpacity>
          </View>
        ...
        <View style={styles.scrollView}>
          {this.props.searchData ?
            ...
            <FlatList style={styles.scrollView}
              ...
              renderItem={({ item }) => this.MapDetailPage_onRenderItem(item)}
            />
          }
        </View>
      </View>
